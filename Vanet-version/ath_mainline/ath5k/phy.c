...
bool ath5k_channel_ok(struct ath5k_hw *ah, u16 freq, unsigned int flags)
{
	...
	if (flags & CHANNEL_2GHZ) {
		...
		if ((freq >= ah->ah_capabilities.cap_range.range_5ghz_min) &&
			...
			//printk("VANET-debug: ah->ah_capabilities.cap_range.range_5ghz_max=%d\n", 
...
//	channel->center_freq = 5745;
	printk("VANET-debug: channel->center_freq=%u\n", channel->center_freq);
		 ...
		 u8 txpower)
{
	...
	ee_mode = ath5k_eeprom_mode_from_channel(channel);
	printk("VANET-debug: ee_mode=%d, and th txpower(in half dB)=%d\n", ee_mode, txpower);
	if (ee_mode < 0) {
		ATH5K_ERR(ah,
			"invalid channel: %d\n", channel->center_freq);
		return -EINVAL;
	...
	if (!ah->ah_txpower.txp_setup ||
		...
		ret = ath5k_setup_channel_powertable(ah, channel,
							ee_mode, type);
		printk("VANET-debug: in Calculate the powertable, ret=%d\n", ret);
		if (ret)
			return ret;
	}
	...
	printk("VANET-debug: in register writing\n");
		      ...
		      u8 mode, bool fast)
{
	 ...
	 */
	printk("VANET-debug: only phy_init, ah->ah_txpower.txp_cur_pwr=%d\n", ah->ah_txpower.txp_cur_pwr);
